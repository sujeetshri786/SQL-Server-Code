...
(Select isnull(sum(LoanamountDisbursed),0) from clientloansubscription with(nolock) where disbursementdate between '2025-04-01' and '2025-04-21' 
...
where weeklytransactiondate between '2025-04-01' and '2025-04-30' and ll.RegionID = '10'
...
--Progress Report staff Wise, Proposal detail, Disbursement, Collection

	select D.Zone,D.districtName ,A.BranchID,d.Branchname,d.CHID,(select Staffname from fieldstaff with(nolock) where Staffid=d.chid) as CHIB_Name,
	A.StaffID,A.StaffName,Position, isnull(NumCenter,0) as NUm_Centers,isnull(h.Numpro_own,0) Numpro_own,isnull(h.LoanAmountpro_OWN,0) LoanAmountpro_OWN,   
	isnull(b.Numpro,0) as BCproposal, isnull(b.LoanAmount,0) as BCproposal_AMT, isnull(g.NumproSidbi,0) as SIDBI_proposal,
	...
	from (select Branchid,Staffid,StaffName ,case                                     
	...
	where f.Designation in('12','13','14','15','47','46') and exitdate is null
	) A left join 
	(
	...
	where c.proposaldate between    '2026-02-01' and '2026-02-28'      and c.productcategory like '%BC%' 
	group by c.entryby
	...
	where cls.disbursementdate between    '2026-02-01' and '2026-02-28'       
	...
	from clientloanproposal c (nolock) 
	where c.proposaldate between    '2026-02-01' and '2026-02-28'       and c.productcategory like '%SIDBI%' 
	group by c.entryby
	...
	from clientloanproposal c (nolock) 
	where c.proposaldate between    '2026-02-01' and '2026-02-28'       and c.productcategory like '%IGL%' 
	group by  c.entryby
	...
	from clientloanproposal c (nolock) 
	where c.proposaldate between    '2026-02-01' and '2026-02-28'        
	group by c.entryby
	...
	from clientloanproposal c (nolock) 
	where c.proposaldate between    '2026-02-01' and '2026-02-28'       and c.productcategory like '%FL%' 
	group by  c.entryby
	) K on A.StaffID=K.Staffid left Join

	(select c.entryby as Staffid ,count(c.clientid) Adding
	from clients c (nolock) 
	where cast(c.joindate as date) between    '2026-02-01' and '2026-02-28'       
	group by c.entryby,left(clientid,5)
	) e     on A.Staffid=e.Staffid left join
	(select Staffid,count(centerid)NumCenter from centers (nolock) where dissolveddate is null group by Staffid
	)O on A.Staffid=o.Staffid left join

	(select Staffid,sum(principalcollected) as Collected from loanledger (nolock) where weeklytransactiondate between    '2026-02-01' and '2026-02-28'       group by Staffid
	) p on p.Staffid=a.Staffid inner join

	Branches D on A.Branchid=D.Branchid
	where d.DissolvedDate is null and D.RegionID in ('05')
