...
inner join Targets As T With (Nolock) On LU.targetID=T.TargetID and LEFT(LU.ClientLoanID,2) in ('24') and CONVERT(varchar(10),LU.loandisbursementdate,23)>=CONVERT(varchar(10),GETDATE()-15,23)
...
inner join Targets As T With (Nolock) On LU.targetID=T.TargetID and LEFT(LU.ClientLoanID,2) in ('24') and CONVERT(varchar(10),LU.loandisbursementdate,23)>=CONVERT(varchar(10),GETDATE()-45,23) 
inner Join Branches As B With (Nolock) On T.BranchID=B.BranchID and B.DissolvedDate is null 
inner join Clients AS C With (Nolock) On T.TargetID=C.TargetID and C.DropOutStatus=0
inner join Centers As CC With (Nolock) On T.CenterID=CC.CenterID and CC.DissolvedDate is null
inner join FieldStaff as Fs With (Nolock) On CC.StaffID=Fs.StaffID and Fs.ExitDate is null
...
inner join Targets As T With (Nolock) On LU.targetID=T.TargetID and LEFT(LU.ClientLoanID,2) in ('24') and CONVERT(varchar(10),LU.loandisbursementdate,23)>=CONVERT(varchar(10),GETDATE()-60,23)
inner Join Branches As B With (Nolock) On T.BranchID=B.BranchID and B.DissolvedDate is null  
inner join Clients AS C With (Nolock) On T.TargetID=C.TargetID and C.DropOutStatus=0
inner join Centers As CC With (Nolock) On T.CenterID=CC.CenterID and CC.DissolvedDate is null
group by B.DistrictName,B.BranchID,B.BranchName,CC.CenterID,CC.CenterName,C.ClientID,T.TargetName,LU.ClientLoanID,LU.loanamountdisbursed,
