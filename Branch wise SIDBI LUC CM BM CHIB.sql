--Branch wise SIDBILUCCM/BM/CHIB Appcashpor
...
--CONVERT(varchar (10),loandisbursementdate ,23)<= CONVERT(varchar(10),GETDATE()-15,23) and 
 ClientLoanID in ( select ClientLoanID  from ClientLoanProposal where PrincipalOutstanding >1)
  and loandisbursementdate between '2023-04-01' and '2023-11-31'
  and LEFT(clientloanid,2)='05' --and substring(clientloanid,16,2) in(select productid from products where type like'%SIDBI%')
